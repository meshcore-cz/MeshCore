...
void buildBroadcastDatagram(const uint8_t selfId[NODE_ID_LEN], const uint8_t datagramId[DATAGRAM_ID_LEN],
                            uint8_t ttl, uint16_t protocol,
  ...
  emitUint(out, KEY_SOURCE); emitBstr(out, selfId, NODE_ID_LEN);
  emitUint(out, KEY_DEST); emitBstr(out, zero, NODE_ID_LEN);
  emitUint(out, KEY_TTL); emitUint(out, ttl);
  emitUint(out, KEY_PROTOCOL); emitUint(out, protocol);
  emitUint(out, KEY_PAYLOAD); emitBstr(out, payload, payloadLen);
}
...
#elif defined(NRF52_PLATFORM)
  if (sd_rand_application_vector_get(out, TRANSFER_ID_LEN) != NRF_SUCCESS) {
    for (size_t i = 0; i < TRANSFER_ID_LEN; i++) out[i] = (uint8_t)random(0, 256);
  }
#else
